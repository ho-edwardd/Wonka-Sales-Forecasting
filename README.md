## Overview
This project uses a two-segment (two latent classes) Burr XII or Weibull-Gamma model to forecast ticket sales for the movie "Wonka" after its release. The model incorporates a spike at 0 to account for Hardcore Never Buyers (HCNB) and segments the population into two latent classes to capture different viewing behaviors, such as the Christmas break effect for younger audiences and general weekend trends for older audiences. The power of this model comes from the fact that the model is able to achieve ~14% MAPE, 0.3% cumulative MAPE, and ~99% variance explained with static, determinsitic features. This allows for a robust, lightweight model that could be extrapolated out to other films due to foundation being a flexible probability distribution. This displays the power of a great, simplistic model built with a logical approach, with each aspect having important story telling properties to explain the customers and their underlying behaviors. The selection of the Weibull-Gamma (Burr XII) distribution was predetermined based on underlying behavioral assumptions prior to any data fitting or modeling. Because we suspect moviegoers exhibit a continuously decreasing probability of viewing a film over time, they possess non-constant hazard functions; this makes the Weibull model vastly superior to a standard exponential baseline. To account for varying propensities across the population (unobserved heterogeneity), we implemented a mathematically sound Weibull-Gamma mixture. If time-dependence proves negligible, this model conveniently collapses into a Pareto II distribution. Ultimately, this approach highlights the efficacy of designing statistical architectures around fundamental customer behaviors. The entirety of modeling and analysis could be found in the Excel xlxs file.
...
2. 2-seg WG (Burr XII) HCNB: Model parameters and predictions for the two-segment Burr XII/Weibull-Gamma model using the whole dataset.
3. Hold-out Testing (40 days): Evaluates the model's performance on a last 40-day as a hold-out dataset and only treats the first 52 days as being avaliable for use in parameter calibration (training data). The functional structure of the model remains consistent (2-segment Burr XII HCNB).
